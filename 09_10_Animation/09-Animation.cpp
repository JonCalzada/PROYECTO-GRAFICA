...
glm::vec3 position(1.0f,0.02f, 5.0f);
...
bool Start() {
	...
	baseP = new Model("models/IllumModels/PlumaCompleta.fbx");
	plumaP = new Model("models/IllumModels/PlumaP_.fbx");
	moon = new Model("models/IllumModels/moon.fbx");
	gridMesh = new Model("models/IllumModels/grid.fbx");
	character01 = new AnimatedModel("models/IllumModels/animaJons.fbx");
	character02 = new AnimatedModel("models/IllumModels/animateJons.fbx");
	char3 = new AnimatedModel("models/IllumModels/animaJons.fbx");
	char4 = new AnimatedModel("models/IllumModels/lata.fbx");
	fabrica = new Model("models/IllumModels/fabricas.fbx");
...
bool Update() {
	...
	// Cubemap (fondo)
	{
	 ...
	 {/// AQUI VA LA CASA
		...
		baseP->Draw(*mLightsShader);
		

		 model = glm::mat4(1.0f);

		// Actividad 5.1
		// Efecto de puerta corrediza  ///////////x,yz
		 model = glm::translate(model, glm::vec3(0.0f  , 3.0f, -2.9f));
		//model = glm::translate(model, glm::vec3(0.418f +door_offset , 0.0f, 6.75f));
		
		// Efecto de puerta con bisagra
		 //model = glm::rotate(model, glm::radians(door_rotation), glm::vec3(0.0f, 4.0f, 0.0f));

		 model = glm::rotate(model, glm::radians(door_rotation), glm::vec3(0.0f, 0.0f, 1.0f));
		 mLightsShader->setMat4("model", model);
		 plumaP->Draw(*mLightsShader);
	...
	
	{
		...
		proceduralShader->setFloat("height", 8.0f);
	...
	
	{
		...
		//model = glm::mat4(1.0f);
		//model = glm::translate(model, glm::vec3(3.0, 0.0f, 0.0f)); // translate it down so it's at the center of the scene
		//model = glm::rotate(model, glm::radians(rotateCharacter), glm::vec3(0.0, 1.0f, 0.0f));
		...
		//Aguacate
		model = glm::mat4(1.0f);
		model = glm::translate(model, glm::vec3(5.0, 0.2f, 0.0f)); // translate it down so it's at the center of the scene
		model = glm::rotate(model, glm::radians(rotateCharacter), glm::vec3(1.0, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(0.0001f, 0.0001f, 0.001f));	// it's a bit too big for our scene, so scale it down
		dynamicShader->setMat4("model", model);
